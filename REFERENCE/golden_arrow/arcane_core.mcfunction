...
# # Destroy Golden Arrow's summoned creatures if they're not present anymore.
execute as @e[type=goldark:ga_shadow] unless entity @e[type=stray, tag=G_Arrow] run kill @s

execute as @e[type=stray, tag=g_arrow:gminion_summon] unless entity @e[type=stray, tag=G_Arrow] run kill @s