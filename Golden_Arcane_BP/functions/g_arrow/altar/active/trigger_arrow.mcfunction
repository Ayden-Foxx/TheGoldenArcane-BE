...
scoreboard players add @s goldark.global.self_clock 1
...
execute if score @s goldark.global.self_clock matches 100 run titleraw @a[r=16] actionbar {"rawtext": [{"translate": "gdark.g_arrow.summon.warn_near"}]}

execute if score @s goldark.global.self_clock matches 100 run tellraw @a[rm=16] {"rawtext": [{"text": "§8§l[...] §r"}, {"translate": "gdark.g_arrow.summon.warn_far"}]}

# The Darkness starts to take shape...
execute if score @s goldark.global.self_clock matches 100 run playsound wither.spawn @a[r=16] ~ ~ ~ 1 0.5
...
execute if entity @s[scores={goldark.global.self_clock=100..}] run tp @s ~ ~ ~ ~8

# Create particles around it. These will also rotate with the dummy itself.
execute if entity @s[scores={goldark.global.self_clock=100..}] run particle minecraft:blue_flame_particle ^1 ^ ^1
execute if entity @s[scores={goldark.global.self_clock=100..}] run particle minecraft:blue_flame_particle ^-1 ^ ^1

# Also create particles on the skull's position
execute if entity @s[scores={goldark.global.self_clock=100..}] at @e[type=goldark:altar_spot, c=1] run particle minecraft:critical_hit_emitter


# * AT 160 TICKS & ONWARDS: (3 secs later)
# A second flame begins playing above the altar before the last countdown ends.
execute if entity @s[scores={goldark.global.self_clock=160..}] run particle minecraft:basic_flame_particle ^ ^ ^0.25

execute if score @s goldark.global.self_clock matches 160 run playsound mob.evocation_fangs.attack @a[r=16] ~ ~ ~ 1.5 0.5
...
execute if score @s goldark.global.self_clock matches 200 run kill @s